        ...
        if host in config['whitelisting']and request_method in arrayName and is_within_time_range(config):
            if url in cache_data:
                ...
                if current_time - timestamp <= config['cache_time']:
                   response_data = cached_data['image_data']
                response_header, response_body = response_data.split(b"\r\n\r\n",1)
                ...
                remote_socket.close() 
                client_socket.close()
                if response_data and b'content-type: image/' in response_data.lower():
                    save_image(response_data, host, image_name, url)
                    delete_folder_after_delay(f"cache/{host}", url)
        else:
            Error403(client_socket)
            client_socket.close() 
def save_image(response_data, host, image_name, url):
    response_header,response_body = response_data.split(b'\r\n\r\n',1)
    ...
    if not os.path.exists(folder):
        os.makedirs(folder)
        cache_data[url] = {'image_data': response_data, 'timestamp': time.time()}
...
def proxy_server():
    ...
    server_socket.listen(5)  # Lắng nghe kết nối từ trình duyệt, giới hạn đợi đến 20 kết nối
